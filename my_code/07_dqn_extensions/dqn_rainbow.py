...
class RainbowDQN(nn.Module):
    """
    Combines dueling DQN, Noisy and categorical DQN into one architecture
    """
    def __init__(self, input_shape, n_actions):
        ...
        self.conv = nn.Sequential(
            ...
            nn.ReLU,
            nn.Conv2d(32, 54, kernel_size=4, stride=2),
            nn.ReLU(),
    ...
    def forward(self,x):
        ...
        adv_out = self.fc_adv(conv_out_).view(batch_size, -1, N_ATOMS)
        adv_mean = adv_out.mean(dim=1, keepdim=True)
        return val_out + adv_out - adv_mean

    # both and q_vals fn are used to be able to combine probability distributions
    # into Q-values, without calling the network several times
...
def calc_loss(batch, batch_weights, net, tgt_net, gamma, device="cpu"):
    ...
    proj_distr = common.distr_projection(next_best_distr, rewards, dones, Vmin, Vmax,
        N_ATOMS, gamma)
